...
class _AuthScreenState extends State<AuthScreen> {
  ...
  void _submit() async {
    ...
    try {
      ...
      if (_isLogin) {
        // final userCredentials = await _user.signInWithEmailAndPassword(
        //     email: _enteredEmail.toLowerCase().trim(),
        //     password: _enteredPassword.trim());
        await _user.signInWithEmailAndPassword(
            email: _enteredEmail.toLowerCase().trim(),
            password: _enteredPassword.trim());
      } else {
        // final userCredentials = await _user.createUserWithEmailAndPassword(
        //   email: _enteredEmail.toLowerCase().trim(),
        //   password: _enteredPassword.trim(),
        // );
                          ...
                          TextFormField(
                            ...
                            validator: (value) {
                              if (value == null || value.trim().length < 6) {
                                return 'Please enter a valid password longer than 6';
                              }
                              return null;
                            },
                            onSaved: (value) {
